...
describe("api/auth/reset-password route", () => {
  ...
  it("redirects with rate_limited when limiter blocks request", async () => {
    enforceRateLimitMock.mockReturnValueOnce({ allowed: false, retryAfterSeconds: 7 });
    const request = new Request("http://localhost:3000/api/auth/reset-password", {
      method: "POST",
      body: new FormData(),
    });
    ...
    expect(response.headers.get("location")).toBe("http://localhost:3000/reset-password?error=rate_limited");
  });

  it("redirects with reset_failed when service throws", async () => {
    resetPasswordByTokenMock.mockRejectedValueOnce(new Error("db_error"));
    const formData = new FormData();
    formData.set("token", "token-abcdefghijklmnopqrstuvwxyz");
    formData.set("password", "NewPass123!");
    const request = new Request("http://localhost:3000/api/auth/reset-password", {
    ...
    expect(response.headers.get("location")).toBe(
      "http://localhost:3000/reset-password?token=token-abcdefghijklmnopqrstuvwxyz&error=reset_failed",
    );
  });
});
